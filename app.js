...
const text404 = fs.readFileSync(path.normalize(__dirname + '/demo/index.html'), 'utf8'),
    siteIndex = 'demo/index.html',
    cookingInserts = [
...
app.get('/', async(req, res) => res.send(insertAll(tryReadFile(path.normalize('demo/index.html' + (['/', '/?'].includes(req.url) ? siteIndex : pages[Object.keys(req.query)[0]]))))));

/* Static Files Served */
app.use(express.static(path.normalize('demo/index.html')));
app.use((req, res) => {
